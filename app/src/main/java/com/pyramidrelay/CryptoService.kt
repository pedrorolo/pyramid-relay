...
class CryptoService(private val context: Context? = null) {
    ...
    /** Streaming encryption to a file — avoids OOM on large files. */
    fun encryptCompressedFile(inputFile: java.io.File, outputFile: java.io.File, privateKey: PrivateKey) {
        require(privateKey.algorithm.equals(RSA, ignoreCase = true)) { "Private key must be RSA" }
        val aes = KeyGenerator.getInstance("AES").apply { init(256) }.generateKey()
        val nonce = ByteArray(12).also { SecureRandom().nextBytes(it) }
        val wrapped = Cipher.getInstance("RSA/ECB/PKCS1Padding").run {
        ...
        outputFile.outputStream().use { fos ->
            fos.write(byteArrayOf('P'.code.toByte(), '2'.code.toByte(), 'P'.code.toByte(), 'E'.code.toByte(), 1))
            fos.write(nonce)
            fos.write(intBytes(wrapped.size))
            fos.write(wrapped)
            val buffer = ByteArray(8192)
            inputFile.inputStream().use { input ->
                var n = input.read(buffer)
                while (n > 0) {
                    fos.write(cipher.update(buffer, 0, n))
                    n = input.read(buffer)
                }
            }
