...
data class BleMetaPayload(
    ...
    val fileSize: Long,
    val fileName: String,
    val keyId: ByteArray = ByteArray(4),
    val deviceId: ByteArray = ByteArray(16)
) {
    companion object {
        // Fixed prefix size (everything except the variable-length fileName):
        // fileId 16B + version 4B + hash 32B + size 4B + keyId 4B + deviceId 16B + nameLen 2B
        const val FIXED_SIZE = 16 + 4 + 32 + 4 + 4 + 16 + 2
        ...
        fun fromBytes(data: ByteArray): BleMetaPayload? {
            ...
            offset += 4
            val keyId = data.copyOfRange(offset, offset + 4); offset += 4
            val deviceId = data.copyOfRange(offset, offset + 16); offset += 16
            ...
            return BleMetaPayload(fileId, version, fileHash, fileSize, fileName, keyId, deviceId)
        }
    }
    ...
    fun toBytes(): ByteArray {
        ...
        offset += 4
        keyId.copyInto(out, offset); offset += 4
        deviceId.copyInto(out, offset); offset += 16
