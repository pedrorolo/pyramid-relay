...
class PeerSyncIntegrationTest {
    ...
    private fun publish(content: ByteArray): Pair<BroadcastEntity, KeyPair> {
        ...
        val entity = BroadcastEntity(fileId, "payload.bin", "application/octet-stream", file.absolutePath, hash, content.size.toLong(), compressedSize, version, publicKey, "sk_$fileId", "", Role.ORIGINATOR, 0L, 0L)
        coEvery { broadcastDaoA.getById(fileId) } returns entity
        return entity to keys
    }

    private fun subscribe(entity: BroadcastEntity) {
        coEvery { subscriptionDaoB.getAll() } returns listOf(SubscriptionEntity(entity.fileId, entity.publicKey, entity.fileName, null, null, 0L, null, null, null))
    ...
    fun `advertisement protocol preserves file hash version and key id`() = runBlocking {
        ...
        val parsed = BleAdvertisement.fromServiceData(advertisementsA[entity.fileId]!!)!!
        assertArrayEquals(cryptoA.fileIdHash(entity.fileId), parsed.fileIdHash)
    ...
    fun `subscriber discovery reaches GATT using QR public key identity`() = runBlocking {
        ...
        subscribe(entity)
        engineA.startAdvertising(entity)
        coEvery { bleCentralB.readMeta(any(), any()) } returns engineA.buildMetaPayload(entity.fileId)
        assertFalse(engineB.handleDiscoveredDevice("AA:BB:CC:DD:EE:01", advertisementsA[entity.fileId]!!).also {
    ...
    fun `stale advertisement does not contact peer`() = runBlocking {
        ...
        coEvery { subscriptionDaoB.getAll() } returns listOf(SubscriptionEntity(entity.fileId, entity.publicKey, entity.fileName, 2, "/local", 0L, 2, 0L, null))
        engineA.startAdvertising(entity)
        assertFalse(engineB.handleDiscoveredDevice("AA:BB:CC:DD:EE:02", advertisementsA[entity.fileId]!!))
