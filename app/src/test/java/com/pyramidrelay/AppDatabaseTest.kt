...
class AppDatabaseTest {
    ...
    fun `broadcastDao upsert and getAll returns entity`() = runTest {
        val broadcast = BroadcastEntity(
            "file-1", "test.txt", null, "text/plain", "/path", "hash1",
    ...
    fun `broadcastDao upsert replaces existing entity`() = runTest {
        val v1 = BroadcastEntity("f1", "a.txt", null, "t", "/p", "h", 100, 100, 1, "pk", null, "s", Role.ORIGINATOR, 0, 0)
        val v2 = BroadcastEntity("f1", "b.txt", null, "t2", "/p2", "h2", 200, 200, 2, "pk2", "sk2", "s2", Role.ORIGINATOR, 1, 2)
    ...
    fun `broadcastDao getById returns entity`() = runTest {
        val broadcast = BroadcastEntity("f1", "a", null, "t", "/p", "h", 100, 100, 1, "pk", null, "s", Role.ORIGINATOR, 0, 0)
        broadcastDao.upsert(broadcast)
    ...
    fun `broadcastDao updateVersion modifies version and hash`() = runTest {
        val broadcast = BroadcastEntity("f1", "a", null, "t", "/p", "old_hash", 100, 100, 1, "pk", null, "s", Role.ORIGINATOR, 0, 0)
        broadcastDao.upsert(broadcast)
    ...
    fun `broadcastDao changeFlow emits on upsert`() = runTest {
        broadcastDao.upsert(BroadcastEntity("f1", "a", null, "t", "/p", "h", 100, 100, 1, "pk", null, "s", Role.ORIGINATOR, 0, 0))
        advanceUntilIdle()
    ...
    fun `subscriptionDao upsert and getAll returns entity`() = runTest {
        val sub = SubscriptionEntity("sub-1", "pk1", "file.txt", null, 3, "/path", 1000L, 5, 2000L, 4)
    ...
    fun `subscriptionDao upsert replaces existing entity`() = runTest {
        val v1 = SubscriptionEntity("s1", "pk", "old.txt", null, null, null, 0, null, null, null)
        val v2 = SubscriptionEntity("s1", "pk2", "new.txt", null, 1, "/p", 1, null, null, null)
    ...
    fun `subscriptionDao getById returns entity`() = runTest {
        subscriptionDao.upsert(SubscriptionEntity("s1", "pk", "f", null, null, null, 0, null, null, null))
    ...
    fun `subscriptionDao handles null fields correctly`() = runTest {
        subscriptionDao.upsert(SubscriptionEntity("s1", "pk", null, null, null, null, 100, null, null, null))
        val result = subscriptionDao.getById("s1")!!
    ...
    @Test
    fun `appDatabase onUpgrade recreates tables`() = runTest {
        val broadcast = BroadcastEntity("f1", "a", null, "t", "/p", "h", 100, 100, 1, "pk", null, "s", Role.ORIGINATOR, 0, 0)
        broadcastDao.upsert(broadcast)
        ...
        // onUpgrade drops and recreates tables; data is lost
        assertEquals(0, broadcastDao.getAll().size)

        // New data can be inserted and compressedSize is stored correctly
        val broadcast2 = BroadcastEntity("f2", "b", null, "t2", "/p2", "h2", 200, 300, 2, "pk2", null, "s2", Role.ORIGINATOR, 0, 0)
        broadcastDao.upsert(broadcast2)
        val all = broadcastDao.getAll()
        assertEquals(1, all.size)
        assertEquals(300, all[0].compressedSize)
    }

    @Test
    fun `broadcastDao multiple entities roundtrip`() = runTest {
        val b1 = BroadcastEntity("f1", "a.txt", null, "t", "/p1", "h1", 100, 100, 1, "pk1", null, "s1", Role.ORIGINATOR, 10, 20)
        val b2 = BroadcastEntity("f2", "b.txt", null, "t2", "/p2", "h2", 200, 200, 2, "pk2", "sk2", "s2", Role.RELAY, 30, 40)
    ...
    fun `subscriptionDao multiple entities roundtrip`() = runTest {
        val s1 = SubscriptionEntity("s1", "pk1", "f1", null, null, null, 0, null, null, null)
        val s2 = SubscriptionEntity("s2", "pk2", "f2", null, 1, "/p", 100, 2, 200, 1)
