...
class ModelsTest {
    ...
    fun `spec 8 - BLE META_CHAR payload is 122 bytes`() {
        assertEquals(78, BleMetaPayload.FIXED_SIZE)
    }

    @Test
    fun `spec 8 - BLE META_CHAR layout fileId16 version4 sig64 fileHash32 size4 deviceId16`() {
        val fileId = ByteArray(16) { (it + 1).toByte() }
        val version = 1
        val fileHash = ByteArray(32) { (it + 30).toByte() }
        val fileSize = 1024L
        val deviceId = ByteArray(16) { (it + 50).toByte() }

        val payload = BleMetaPayload(fileId, version, fileHash, fileSize, "test.bin", ByteArray(4), deviceId)
        val bytes = payload.toBytes()

        // Fixed prefix (no name) = 78 bytes; + 8B name = 86
        ...
        offset += 4
        offset += 4 // keyId
        assertArrayEquals(deviceId, bytes.copyOfRange(offset, offset + 16)); offset += 16
    }
    ...
    fun `spec 8 - BleMetaPayload toBytes and fromBytes roundtrip`() {
        ...
        val deviceId = ByteArray(16) { (it + 100).toByte() }

        val payload = BleMetaPayload(fileId, version, fileHash, fileSize, "report.pdf", ByteArray(4), deviceId)
        ...
        assertArrayEquals(deviceId, restored.deviceId)
    }
    ...
    fun `spec 4 - SubscriptionEntity lastNotifiedVersion tracks dedupe`() {
        val s1 = SubscriptionEntity("id", "pk", null, null, null, null, 0L, null, null, null)
        assertNull(s1.lastNotifiedVersion)
        val s2 = SubscriptionEntity("id", "pk", null, null, null, null, 0L, 1, 100L, 1)
